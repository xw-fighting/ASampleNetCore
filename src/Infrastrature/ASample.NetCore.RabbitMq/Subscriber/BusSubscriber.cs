...
namespace ASample.NetCore.RabbitMq.Subscriber
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        public IBusSubscriber SubscribeCommand<TCommand>(string @namespace = null, string queueName = null, Func<TCommand, ASampleException, IRejectedEvent> onError = null) where TCommand : ICommand
        {
        ...
        public IBusSubscriber SubscribeEvent<TEvent>(string @namespace = null, string queueName = null, Func<TEvent, ASampleException, IRejectedEvent> onError = null) where TEvent : IEvent
        {
        ...
        private async Task<Acknowledgement> TryHandleAsync<TMessage>(TMessage message,
            ...
            Func<Task> handle, Func<TMessage, ASampleException, IRejectedEvent> onError = null)
        {
            var currentRetry = 0;
            ...
            return await retryPolicy.ExecuteAsync<Acknowledgement>(async () =>
            {
                ...
                using (scope)
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        if (exception is ASampleException dShopException && onError != null)
                        {
